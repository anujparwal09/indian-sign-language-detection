"""
Indian Sign Language Detection - Streamlit Cloud Compatible Version
Uses streamlit_webrtc for webcam access on cloud platforms
"""
...
# ====================== SETUP ======================
st.set_page_config(layout="wide", page_title="Indian Sign Language Detection")
st.markdown("""
    <style>
        .css-18e3th9 {
            padding-top: 0rem;
...
# ====================== UI LAYOUT ======================
st.markdown("<h1 style='text-align: center;'>🖐️ Indian Sign Language Detection</h1>", unsafe_allow_html=True)
...
with col_controls:
    ...
    text_box = st.empty()
    text_box.text_area("Output", value=st.session_state.text_output, height=100, max_chars=1000, disabled=True)
...
def process_frame(frame):
    ...
    if hands:
        ...
        if aspectRatio > 1:
            k = imgSize / (y_max - y_min) if (y_max - y_min) > 0 else 1
            wCal = math.ceil(k * (x_max - x_min))
            imgResize = cv2.resize(imgCrop, (wCal, imgSize))
            wGap = math.ceil((imgSize - wCal) / 2)
            imgWhite[:, wGap:wGap + wCal] = imgResize
        else:
            k = imgSize / (x_max - x_min) if (x_max - x_min) > 0 else 1
...
with col_camera:
    class VideoProcessor:
        def recv(self, frame):
            ...
            imgOutput = img.copy()
            detector = HandDetector(maxHands=2, detectionCon=0.8, minTrackCon=0.5)
            hands, img_det = detector.findHands(img)
            ...
            if hands:
                ...
                if imgCrop.size > 0:
                    ...
                    processed_img, _ = preprocess_image(imgWhite)
                    prediction = model.predict(processed_img, verbose=0)
                    index = np.argmax(prediction[0])
                    confidence = prediction[0][index]
                    predicted_char = labels[index]
...
st.info("✅ Cloud-compatible version using WebRTC for webcam access")
